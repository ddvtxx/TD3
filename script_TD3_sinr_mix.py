...
for i_loop in range(6):
    ...
    for i_episode in range(episode):
        ...
        for i_iteration in range(max_iteration):
            action_pre_s = TD3_agent_s.choose_action(observation[0], train=False)
            action_pre_m = TD3_agent_m.choose_action(observation[1:], train=False)
            ...
            TD3_agent_s.remember(observation[0], action_0[0], reward, observation_[0], done)
            TD3_agent_s.learn()
            TD3_agent_m.remember(observation[1:], action_0[1:], reward,observation_[1:], done)
            TD3_agent_m.learn()

            actor_loss_s = TD3_agent_s.get_actor_loss()
            critic_loss_s = TD3_agent_s.get_critic_loss()
            actor_loss_history_s.append(actor_loss_s)
            critic_loss_history_s.append(critic_loss_s)

            actor_loss_m = TD3_agent_m.get_actor_loss()
            critic_loss_m = TD3_agent_m.get_critic_loss()
