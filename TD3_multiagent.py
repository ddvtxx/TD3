...
for i_loop in range(10):
    ...
    for i in range(4):
        TD3_agent=globals()[f"TD3_agent_{i}"] = TD3(
        ...
        agent_array.append(TD3_agent)
        create_directory('./TD3_'+str(i)+'/',sub_paths=['Actor', 'Target_actor', 'Critic', 'Target_critic'])
    ...
    for i_episode in range(episode):
        ...
        for i in range(max_iteration):
            ...
            for i_agent in range(4):
                TD3_agent = agent_array[i_agent]
                observation = observation_array[i_agent]
                done = False
                action_pre = TD3_agent.choose_action(observation,train=True)
    ...
    dataframe.to_csv("./result/bitrate_td3_multiple_wf_"+str(i_loop)+".csv", index=False,sep=',')