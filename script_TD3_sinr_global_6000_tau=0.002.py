...
for i_loop in range(6):
    ...
    TD3_agent = TD3(alpha=1e-4, beta=2e-4,numSenario=numSenario,numAPuser=numAPuser,numRU=numRU,
        ...
        critic_fc4_dim=2**6,
        ckpt_dir='./TD3/',
        gamma=0.99,tau=0.002,action_noise=1e-5,max_size=100000,batch_size=128)
    ...
    for i_episode in range(episode):
        ...
        if i_episode % 50 == 0 and i_loop%2 == 0:
            ...
            dataframe.to_csv("./result/TD3_actor_loss_sinr_single_global_6000_tau0.002_loop"+str(i_loop)+"_epis"+str(i_episode)+".csv", index=False,sep=',')
            dataframe=pd.DataFrame({'bitrate':critic_loss_history})
            dataframe.to_csv("./result/TD3_critic_loss_sinr_single_global_6000_tau0.002_loop"+str(i_loop)+"_epis"+str(i_episode)+".csv", index=False,sep=',')

    dataframe=pd.DataFrame({'bitrate':system_ave_bitrate_history})
    dataframe.to_csv("./result/TD3_bitrate_sinr_single_global_agent_6000_tau0.002_"+str(i_loop)+".csv", index=False,sep=',')
