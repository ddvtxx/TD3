...
for i_loop in range(6):
    ...
    for i_episode in range(episode):
        ...
        for i_iteration in range(max_iteration):
            ...
            TD3_agent.remember(observation[3], action_0, reward, observation_[3], done)
            TD3_agent.learn()
        ...
        if i_episode % 50 == 0 and i_loop%2 == 0:
            ...
            dataframe.to_csv("./result/TD3_actor_loss_sinr_single_individual_loop"+str(i_loop)+"_epis"+str(i_episode)+".csv", index=False,sep=',')
            dataframe=pd.DataFrame({'bitrate':critic_loss_history})
            dataframe.to_csv("./result/TD3_critic_loss_sinr_single_individual_loop"+str(i_loop)+"_epis"+str(i_episode)+".csv", index=False,sep=',')

    dataframe=pd.DataFrame({'bitrate':system_ave_bitrate_history})
    dataframe.to_csv("./result/TD3_bitrate_sinr_single_individual_"+str(i_loop)+".csv", index=False,sep=',')
