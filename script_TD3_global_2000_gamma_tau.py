...
for i_gamma in [0.99, 0.95, 0.90, 0.95 ]:
    for i_tau in [0.001, 0.002, 0.003]:
        for i_loop in range(6):
            ...
            TD3_agent = TD3(alpha=1e-4, beta=2e-4,numSenario=numSenario,numAPuser=numAPuser,numRU=numRU,
                actor_fc1_dim=2**6,actor_fc2_dim=2**7,actor_fc3_dim=2**7,
                actor_fc4_dim=2**6,
                critic_fc1_dim=2**6,critic_fc2_dim=2**7,critic_fc3_dim=2**7,
                critic_fc4_dim=2**6,
                ckpt_dir='./TD3/',
                gamma=i_gamma,tau=i_tau,action_noise=1e-5,max_size=100000,batch_size=128)
            create_directory('./TD3/',sub_paths=['Actor', 'Target_actor', 'Critic', 'Target_critic'])
            ...
            for i_episode in range(episode):
                ...
                if i_episode % 50 == 0 and i_loop%2 == 0:
                    ...
                    dataframe.to_csv("./result/TD3_actor_loss_global_6000_gamma="+str(i_gamma)+"_tau="+str(i_tau)+"_loop="+str(i_loop)+"_epis"+str(i_episode)+".csv", index=False,sep=',')
                    dataframe=pd.DataFrame({'bitrate':critic_loss_history})
                    dataframe.to_csv("./result/TD3_critic_loss_global_6000_gamma="+str(i_gamma)+"_tau="+str(i_tau)+"_loop="+str(i_loop)+"_epis"+str(i_episode)+".csv", index=False,sep=',')

            dataframe=pd.DataFrame({'bitrate':system_ave_bitrate_history})
            dataframe.to_csv("./result/TD3_bitrate_global_agent_6000_gamma="+str(i_gamma)+"_tau="+str(i_tau)+"_"+str(i_loop)+".csv", index=False,sep=',')
